    ...
    function onDigicom_PayGetHTML($vars,$pg_plugin)
    {
        ...
        try {
            ...
            if ($result['data']['code'] == 100) {
                ...
                $app->enqueueMessage($msg .' خطای '. $result['errors']['code'], 'error');
                return false;
            }
        }
        catch(Exception $e) {
        ...
        if (checkHack::checkString($status)){
            if ($status == 'OK') {
                try {
                    ...
                    ////////////////////////////////////////////////////////////////////////////////////
                    $data = array("merchant_id" => $this->params->get('merchant_id'), "authority" => $Authority, "amount" =>  $price/10000);
                    $jsonData = json_encode($data);
                    ...
                    if ($result['data']['code'] == 100) {
                        ...
                        $db->execute();
                        $trackingCode = $result['data']['ref_id'];
                        $msg= plgDigiCom_PayTrangellZarinpalHelper::getGateMsg($result['data']['code']);
                        $message = "کد پیگیری".$trackingCode;
                        $app->enqueueMessage($msg.'<br/>' .$message, 'message');
                    }
                    else {
                        $msg= plgDigiCom_PayTrangellZarinpalHelper::getGateMsg($result['errors']['code']);
                        $app->enqueueMessage($msg, 'error');
